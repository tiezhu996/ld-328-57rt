...
export default function FoodManage() {
  ...
  const columns: ColumnsType<FoodItem> = [
    ...
    { title: '采购单价', render: (_, r) => (r.unit_price != null ? `¥${formatMoney(r.unit_price)}` : `未填（按 ¥${formatMoney(resolveUnitPrice(null))} 估）`) },
    ...
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      <Drawer open={open} title={editing ? '编辑食品' : '新增食品'} width={420} onClose={() => setOpen(false)} destroyOnClose>
        <Form form={form} layout="vertical" onFinish={onSave} initialValues={{ quantity: 1, unit: '份', shelf_life_days: 7, storage_location: 'fridge', category: 'fresh' }}>
          ...
          >
            <InputNumber min={0} precision={2} step={0.01} style={{ width: '100%' }} placeholder="不填按 15 元/单位估算" />
          </Form.Item>
      ...
      <Modal open={!!detail} title={detail?.item.name} footer={null} onCancel={() => setDetail(null)} width={560}>
        {detail && (
          <Space direction="vertical" size={12} style={{ width: '100%' }}>
            ...
            <Row gutter={16}>
              <Col span={24}>采购单价：{detail.item.unit_price != null ? `¥${formatMoney(detail.item.unit_price)} / ${detail.item.unit}` : `未填写（按 ¥${formatMoney(resolveUnitPrice(null))} / ${detail.item.unit} 估算）`}</Col>
            </Row>
            <Space>
            ...
            <Card size="small" title="历史消耗记录">
              {detail.consumption_records.length ? detail.consumption_records.map((r) => (
                <div key={r.id}>{formatDateTime(r.consumed_at)} - {r.quantity} {detail.item.unit} × ¥{formatMoney(r.unit_price)} = ¥{formatMoney(r.amount)}（{r.user?.name || r.user?.phone}）</div>
      ...
      <Modal open={!!csvText} title="CSV 批量导入" onOk={onImport} onCancel={() => setCsvText('')} okText="导入">
        <Input.TextArea rows={8} value={csvText} onChange={(e) => setCsvText(e.target.value)} placeholder="name,category,quantity,unit,shelf_life_days,storage_location[,unit_price]（单价可选，不填按 15 元/单位估算）" />
      </Modal>
    </Space>
  );
}
